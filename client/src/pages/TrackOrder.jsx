...
export default function TrackOrder() {
    ...
    if (!orderId) return <div className="min-h-screen pt-32 text-center text-white">No Order ID provided.</div>;
    if (!order) return <div className="min-h-screen pt-32 text-center text-white">Loading order details...</div>;
        ...
        <div className="min-h-screen bg-background py-10">
            <div className="container mx-auto px-4 max-w-2xl">
                ...
                <div className="bg-surface border border-white/5 rounded-2xl p-6 mb-8">
                    <h2 className="text-xl font-bold text-white mb-4 border-b border-white/10 pb-2">Order Details</h2>
                    
                    {/* Customer Info */}
                    <div className="mb-6 space-y-2">
                        <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-3">Customer Information</h3>
                        ...
                        <div className="space-y-3">
                            {order.items && order.items.map((item, idx) => (
                                <div key={idx} className="flex justify-between items-center bg-black/20 p-3 rounded-lg">
                                    ...
                                    <span className="text-gray-300 font-mono">₹{item.price * item.quantity}</span>
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                <div className="bg-surface border border-white/5 rounded-2xl p-8 relative overflow-hidden">
                    ...
                    {/* Progress Bar Line */}
                    <div className="absolute left-8 md:left-12 top-24 bottom-12 w-1 bg-white/10">
                        <motion.div
                ...
                {order.status === 'delivered' && (
                    ...
                    >
                        ...
                        <p className="text-gray-400 mb-4">Thank you for your order. Enjoy your meal!</p>
                ...
                <div className="mt-8 text-center">
                    <Link
                        to="/support"
                        className="inline-flex items-center gap-2 text-gray-400 hover:text-white transition-colors text-sm border border-white/10 px-4 py-2 rounded-full hover:bg-white/5"
                    >
